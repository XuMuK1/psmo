...
class KalmanFilter:
    ...
    def smoothSignal(self, filteredSignal, errs, aprSignal, aprErrs, K=None, estLag1=False, returnGains=False):
        ...
        for t in np.arange(errs.shape[-1]-2,-1,-1):           
            ...
            if(estLag1):
                #if(t<errs.shape[-1]-2):
                lag1[...,t] =  smoothingGain@smoothedErrs[...,t+1]#errs[...,t]@smoothingGain.T + gains[-2]@(lag1[...,t+1] - self.A@errs[...,t+1])@smoothingGain.T
    ...
    def fit(self, signal, Niter=1000, 
            fixA=False, 
            fixB=False, 
            fixRx=False, 
            fixRy=False, 
        ...
        for i in tqdm(np.arange(Niter)):
            ...
            if(not fixRy):
                ...
                - self.A @ Yp1Y.T
                - Yp1Y @ self.A.T
                + self.A @ YYnolast @ self.A.T  )/(signal.shape[-1]-1)
